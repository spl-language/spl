...
public final class SplLauncher {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (file == null) {
            // @formatter:off
            source = Source.newBuilder(TONE, new InputStreamReader(System.in), "<stdin>").build();
            // @formatter:on
        } else {
            source = Source.newBuilder(TONE, new File(file)).build();
        }
    ...
    private static int executeSource(Source source, InputStream in, PrintStream out, Map<String, String> options) {
        Context context;
        try {
            context = Context.newBuilder(TONE).in(in).out(out).options(options).build();
        ...
        try {
            ...
            if (context.getBindings(TONE).getMember("main") == null) {
                System.err.println("No def main() in TONE source file.");
                return 1;
            }
            if (!result.isNull()) {

 ...
 * {@link ToneLogicalNotNode negate} the {@code ==} operator.
 */
@NodeInfo(shortName = "==")
public abstract class ToneEqualNode extends ToneBinaryNode {
    ...
    @Specialization
    protected boolean equal(ToneNull left, ToneNull right) {
        /* There is only the singleton instance of ToneNull, so we do not need equals(). */
        return left == right;
    }
